...
router.post("/webhooks", (req, res) => {
  ...
  if (!event || !url) {
    return res.status(400).json({ error: "event_and_url_required" });
  }

  const allowed = ["run.finished", "run.failed"];
  if (!allowed.includes(event)) {
    return res.status(400).json({ error: "invalid_event", allowed });
  }
  ...
  return res.json({ ok: true, id: hook.id });
});

// Config (RAG + Chat Replay)
router.get("/config", (req, res) => {
  const rid = req.id;
  logInfo("LT admin config get", { rid, ip: req.ip });
  return res.json(Storage.getConfig());
});
...
router.put("/config", (req, res) => {
  ...
  if (chatReplayEnabled != null && typeof chatReplayEnabled !== "boolean") {
    return res
      .status(400)
      .json({ error: "invalid_value", message: "chatReplayEnabled must be boolean" });
  }
