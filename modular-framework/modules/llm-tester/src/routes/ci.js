...
import fetch from "node-fetch"; // On Node 18+, you can drop this and use the global fetch.
import { logInfo, logError } from "../logger.js";

const router = Router();

/**
 * Executes a suite, aggregates results, and commits a JSON report via GitHub.
 * Note: This writes a report file CI can consume.
 */
router.post("/github/check", async (req, res) => {
  ...
  const {
    ...
    branch = "main",
    suite,
    path: reportPath = "reports/llm-tester/report.json",
  } = req.body || {};
  ...
  try {
    ...
    if (!execResp.ok) {
      logError(
        "LT CI suite execute failed",
        { rid, status: execResp.status, body: text.slice(0, 500) },
        "ci"
      );
    ...
    try {
      exec = JSON.parse(text);
    } catch (e) {
      logError(
        "LT CI suite execute invalid JSON",
        { rid, error: e.message, preview: text.slice(0, 200) },
        "ci"
      );
