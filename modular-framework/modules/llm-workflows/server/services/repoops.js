...
async function callLLM({ model, temperature, messages, schema, corr }) {
  ...
  let content = resp.data?.content || resp.data?.message?.content || '';
  
  if (!content && resp.data?.choices?.[0]?.message?.content) {
    content = resp.data.choices[0].message.content;
  }

  logDebug('repoops_llm_response', { corr, contentLen: content.length, preview: content.slice(0, 300) });
  return content;
}
