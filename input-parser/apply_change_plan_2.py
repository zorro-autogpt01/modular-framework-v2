...
# coding: utf-8
"""
Code-change plan executor with robust content normalization.

Supports:
- Plain JSON strings (already escaped)
- Raw tagged blocks: <#!#!#!> ... </#!#!#!> or symmetric closer
- Base64 variants: content_b64, replacement_b64, match_b64, block_content_b64

Ops:
- write_file | delete_path | rename_path | patch_text | ensure_block
"""
...
def write_text(p: Path, content: str):
    # normalize to LF so diffs are consistent
...
# ---------- normalization (raw tags / base64) ----------

RAW_TAG_OPEN = "<#!#!#!>"
RAW_TAG_CLOSE = "</#!#!#!>"

def _strip_raw_tags(s: str) -> str:
    """
    If the string is wrapped in <#!#!#!> ... </#!#!#!> OR symmetrical <#!#!#!> ... <#!#!#!>,
    strip the tags and return the inner literal content.
    """
    if not isinstance(s, str):
        return s
    if s.startswith(RAW_TAG_OPEN):
        if s.endswith(RAW_TAG_CLOSE):
            return s[len(RAW_TAG_OPEN):-len(RAW_TAG_CLOSE)]
        if s.endswith(RAW_TAG_OPEN):  # symmetric closer style
            return s[len(RAW_TAG_OPEN):-len(RAW_TAG_OPEN)]
    return s

def get_text(d: Dict[str, Any], key: str) -> str:
    """
    Read a string field that may appear as:
      - key (JSON-escaped already)
      - key_b64 (Base64-encoded UTF-8)
      - raw-tagged payload inside key: <#!#!#!> ... </#!#!#!>
    Normalizes to a clean Python str with real newlines.
    """
    b64k = f"{key}_b64"
    if b64k in d and d[b64k] is not None:
        try:
            return base64.b64decode(d[b64k]).decode("utf-8")
        except Exception as e:
            raise ValueError(f"Invalid base64 in field '{b64k}': {e}")
    val = d.get(key, "")
    if val is None:
        return ""
    if not isinstance(val, str):
        return str(val)
    return _strip_raw_tags(val)
...
def patch_once(text: str, p: Dict[str, Any]) -> Tuple[str, int]:
    ...
    # getter for potentially b64/raw-tagged fields
    def g(field: str) -> str:
        return get_text(p, field)
    ...
    if t == "replace_regex":
        ...
        # NOTE: replacement uses Python's re semantics; backslashes are meaningful.
        new_text, n = re.subn(pattern, replacement, text, count=count, flags=re.MULTILINE | re.DOTALL)
        return new_text, n

    if t == "insert_after":
        anchor = g("anchor") or g("match")
        insertion = g("replacement")
        if p.get("newline", True) and insertion and not insertion.endswith("\n"):
            insertion += "\n"
        # respecting regex flag if provided (not in original schema for insert_* but useful)
...
def op_write_file(root: Path, change: Dict[str, Any], dry: bool, backup: bool, assume_yes: bool):
    ...
    content = get_text(change, "content")
...
def op_ensure_block(root: Path, change: Dict[str, Any], dry: bool, backup: bool, assume_yes: bool):
    path = relsafe(root, Path(change["path"]))
    bid = change["block_id"]
    content = get_text(change, "block_content")
    ...
    if start in before and end in before:
        ...
        anchor = get_text(change, "anchor")  # allow raw/b64 anchor too
