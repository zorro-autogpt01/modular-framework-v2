...
Adds macro expansion and regex-lint to avoid JS regex pitfalls in YAML plans.
"""
import argparse
import json
import os
...
# ---------- Macro expansion for tricky literals ----------
# Use these macros in plan strings to avoid YAML/regex escaping hell:
#   [[FSLASH]]                     -> /
#   [[BSLASH]]                     -> \
#   [[RE_FSLASH]]                  -> \/
#   [[RE_BSLASH]]                  -> \\
#   [[TRAILING_SLASHES_RE]]        -> /\/+$/
#   [[TRAILING_BACKSLASHES_RE]]    -> /\\+$/
#   [[NEWLINE]]                    -> \n
#   [[TAB]]                        -> \t
#   [[QUOTE]]                      -> "
#   [[SQUOTE]]                     -> '
# These are applied to all string fields (content, match, replacement, anchors, etc.)
MACROS = {
    '[[FSLASH]]': '/',
    '[[BSLASH]]': '\\',
    '[[RE_FSLASH]]': '\\/',
...
def expand_macros_in_str(s: str) -> str:
...
def expand_macros(obj):
    """Recursively expand macros in any string in the loaded plan object."""
    if isinstance(obj, dict):
        return {k: expand_macros(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [expand_macros(v) for v in obj]
    if isinstance(obj, str):
        return expand_macros_in_str(obj)
    return obj
...
class Validator:
    ...
    def warn_suspicious_js_regex(self, change_id: str, where: str, val: str):
        """
        Warn on common JS regex mistakes in plans, e.g. /\\/+$/ used to trim slashes (matches backslashes).
        Encourage using macros like [[TRAILING_SLASHES_RE]] to avoid escaping issues.
        """
        if not isinstance(val, str) or not val:
            return
        
        # Check for suspicious patterns that likely indicate escaping errors
        suspicious_patterns = [
            ('/\\\\/+$/', 'forward slash trimming', '[[TRAILING_SLASHES_RE]]'),
            ('/\\\\/+$', 'forward slash trimming', '[[TRAILING_SLASHES_RE]]'),
            ('replace(/\\\\/+$/', 'forward slash replace', '[[TRAILING_SLASHES_RE]]'),
            ('replace(/\\/+$/', 'forward slash replace', '[[TRAILING_SLASHES_RE]]'),
        ]
        
        for pattern, desc, macro in suspicious_patterns:
            if pattern in val:
                self.add_warning(
                    change_id,
                    f"{where}: suspicious JS regex for {desc}. Use macro {macro} to avoid escaping issues."
                )
                break
        
        # Warn if someone embedded code-ish transformations in YAML
        if '.replace(' in val and ')' in val and 'replacement' in where.lower():
            self.add_warning(
                change_id, 
                f"{where}: looks like embedded code; replacement values should be literal text. Consider using macros."
            )
...
def validate_change(validator: Validator, change: Dict[str, Any], idx: int):
    ...
    if op == "write_file":
        ...
        if "content" in change:
            validator.warn_suspicious_js_regex(change_id, "write_file.content", str(change["content"]))
        ...
        try:
            ...
            for i, patch in enumerate(patches):
                ...
                # Heuristic regex linting on literals to catch /\\/+$/ mistakes
                if ptype in ("replace_literal", "insert_after", "insert_before", "replace_between"):
                    for field in ("match", "replacement", "anchor", "start", "end"):
                        if field in patch:
                            validator.warn_suspicious_js_regex(patch_id, f"{ptype}.{field}", str(patch[field]))
        ...
        if "block_content" in change:
            validator.warn_suspicious_js_regex(change_id, "ensure_block.block_content", str(change["block_content"]))
...
def print_available_macros():
    """Print all available macros for user reference."""
    print(f"\n{Colors.CYAN}Available Macros:{Colors.RESET}")
    print(f"{Colors.CYAN}{'='*60}{Colors.RESET}")
    for macro, expansion in MACROS.items():
        # Show readable representation
        readable = repr(expansion)[1:-1]  # Remove quotes from repr
        print(f"  {Colors.GREEN}{macro:30}{Colors.RESET} → {readable}")
    print(f"{Colors.CYAN}{'='*60}{Colors.RESET}\n")
...
Examples:
  ...
  %(prog)s --list-macros                # Show available macros
        """
    )
    ap.add_argument("plan", nargs='?', help="Path to plan file (JSON or YAML)")
    ...
    ap.add_argument("--list-macros", action="store_true", help="List available macros and exit")
    args = ap.parse_args()
    ...
    # Require plan file if not listing macros
    if not args.plan:
        ap.print_help()
        sys.exit(1)
    ...
    # Expand macros across the entire plan so author can avoid tricky escaping in YAML
    # (affects write_file.content, patch_text.patch.match/replacement/anchor/etc.)
    info("Expanding macros...")
    plan = expand_macros(plan)
